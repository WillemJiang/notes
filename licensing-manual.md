...
1. 通过二进制的方式，引用三方依赖，代码库中不包含被引用的代码。

  a. 以源代码发布时，不需要提及未包含的被引用三方依赖。

  b. 以二进制形式发布时，如果包含了被引用三方依赖，需要在NOTICE文件，声明三方版本号，源地址以及所属许可协议。如果被包含的三方代码有NOTICE，必须将其NOTICE原样拷贝至项目NOTICE文件中。需要修正LICENSE文件，声明包含三方库版本和所属许可协议。
